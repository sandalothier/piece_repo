package com.brain.piece.web.rest;
...
import com.brain.piece.config.KafkaProperties;
...
class PieceKafkaResourceIT {
    ...
    void setup() {
        ...
        PieceKafkaResource kafkaResource = new PieceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/piece-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/piece-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
